...
public class AlpineFaasServingDriver extends AbstractJobDriver{
    /**
     * Singleton code block
     */
    private static AlpineFaasServingDriver driver=null;
    private SSHTool tool = new SSHTool("192.168.1.4", "root", "wnlof309b507", StandardCharsets.UTF_8);
    public AlpineFaasServingDriver(){initVariables();}
    public synchronized static AlpineFaasServingDriver getInstance() {
        if (driver == null) {
            driver = new AlpineFaasServingDriver();
        }
        return driver;
    }
